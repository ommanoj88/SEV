...
|-------|----------|----------|
| Atomic `decrementAvailableSlots()` exists | ✅ YES | `ChargingStationRepository.java` line 47-49: `@Modifying @Query("UPDATE ChargingStation s SET s.availableSlots = s.availableSlots - 1 WHERE s.id = :id AND s.availableSlots > 0")` |
| Atomic `incrementAvailableSlots()` exists | ✅ YES | `ChargingStationRepository.java` line 52-54: Similar atomic SQL |
| `@DecimalMin/@DecimalMax` on lat/lon | ✅ YES | `ChargingStation.java` lines 52-59: `@DecimalMin("-90.0")`, `@DecimalMax("90.0")` for latitude, `@DecimalMin("-180.0")`, `@DecimalMax("180.0")` for longitude |
| `BigDecimal pricePerKwh` | ✅ YES | `ChargingStation.java` line 62: `private BigDecimal pricePerKwh;` with `precision = 10, scale = 2` |
| `@Version` for optimistic locking | ✅ YES | `ChargingStation.java` line 35: `@Version private Long version;` |
| FuelStation entity missing | ✅ CONFIRMED | No `FuelStation.java` found in codebase |
